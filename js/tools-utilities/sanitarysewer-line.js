// Sanitary sewer line utility polyline tool.
// Drawing lifecycle is handled by js/utils/polyline-tool.js.

(function () {
  if (!window.SitePlanRuntimeReady) {
    console.error('[tools-utilities/sanitarysewer-line] window.SitePlanRuntimeReady is missing. ' +
      'Make sure js/runtime.js and js/tools-utilities/utilities-shared.js load first.');
    return;
  }
  if (!window.SitePlanPolylineTool) {
    console.error('[tools-utilities/sanitarysewer-line] window.SitePlanPolylineTool is missing. ' +
      'Make sure js/utils/polyline-tool.js is loaded before sanitarysewer-line.js.');
    return;
  }
  ...
  window.SitePlanRuntimeReady.then(RT => {
    ...
    const TOOL_ID           = 'sanitarySewerLine';
    const BUTTON_ID         = 'btn-sanitarysewer-line';
    ...
    function fallbackSymbol() {
      return { type: 'simple-line', color: [63, 90, 54, 1], width: 2.2, cap: 'round', join: 'round' };
    }
    ...
    // Polygons that sanitary sewer lines can snap to: proposed/existing
    ...
    const sanitarySewerLineIcon =
      '<svg viewBox="0 0 36 22" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">' +
        '<g class="dm-existing">' +
          '<line x1="3" y1="11" x2="12.3" y2="11" stroke="#3F5A36" stroke-width="3" stroke-linecap="butt"/>' +
          '<line x1="23.7" y1="11" x2="33" y2="11" stroke="#3F5A36" stroke-width="3" stroke-linecap="butt"/>' +
          '<text x="18" y="11.6" font-family="Arial, sans-serif" font-size="7" font-weight="700" fill="#3F5A36" text-anchor="middle" dominant-baseline="middle">SS</text>' +
        '</g>' +
        '<g class="dm-proposed" style="display:none">' +
          '<g fill="#3F5A36">' +
            '<rect x="3" y="9.5" width="4" height="3"/>' +
            '<rect x="8.3" y="9.5" width="4" height="3"/>' +
            '<rect x="23.7" y="9.5" width="4" height="3"/>' +
            '<rect x="29" y="9.5" width="4" height="3"/>' +
          '</g>' +
          '<text x="18" y="11.6" font-family="Arial, sans-serif" font-size="7" font-weight="700" fill="#3F5A36" text-anchor="middle" dominant-baseline="middle">SS</text>' +
        '</g>' +
      '</svg>';
    ...
    const drawing = window.SitePlanPolylineTool.create({
      ...
      buttonId:    BUTTON_ID,
      category:    'utilities',
      label:       'Sanitary sewer line',
      idPrefix:    'sanitarysewerline',
      order:       45,
      proposedMode: true,
      iconApply:   window.SitePlanDrawingMode.iconSwapApply,
      iconClass:   'dm-line36',
      buttonTitle: 'Draw a sanitary sewer line',
      iconHtml:    sanitarySewerLineIcon,
      ...
      applyExtraMetadata: graphic => {
        if (US.ensureSitePlanId) {
          const id = US.ensureSitePlanId(graphic, 'sanitarysewerline');
    ...
    window.startSanitarySewerLineTool    = drawing.start;
    window.SitePlanSanitarySewerLineTool = Object.assign({}, window.SitePlanSanitarySewerLineTool || {}, api);

    if (typeof US.registerTool === 'function') {
      US.registerTool(window.SitePlanSanitarySewerLineTool);
    } else {
      console.warn('[tools-utilities/sanitarysewer-line] Utilities registry unavailable.');
    }

  }).catch(err => {
    console.error('[tools-utilities/sanitarysewer-line] Failed to initialize after runtime ready:', err);
  });
}());
