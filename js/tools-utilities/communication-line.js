// Overhead communication line utility polyline tool.
// Drawing lifecycle is handled by js/utils/polyline-tool.js.

(function () {
  if (!window.SitePlanRuntimeReady) {
    console.error('[tools-utilities/communication-line] window.SitePlanRuntimeReady is missing. ' +
      'Make sure js/runtime.js and js/tools-utilities/utilities-shared.js load first.');
    return;
  }
  if (!window.SitePlanPolylineTool) {
    console.error('[tools-utilities/communication-line] window.SitePlanPolylineTool is missing. ' +
      'Make sure js/utils/polyline-tool.js is loaded before communication-line.js.');
    return;
  }
  ...
  window.SitePlanRuntimeReady.then(RT => {
    ...
    const TOOL_ID           = 'communicationLine';
    const BUTTON_ID         = 'btn-communication-line';
    ...
    // Polygons that communication lines can snap to: proposed/existing
    ...
    const communicationLineIcon =
      '<svg viewBox="0 0 36 22" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">' +
        '<g class="dm-existing">' +
          '<line x1="3" y1="11" x2="9.4" y2="11" stroke="#CC8400" stroke-width="3" stroke-linecap="butt"/>' +
          '<line x1="26.6" y1="11" x2="33" y2="11" stroke="#CC8400" stroke-width="3" stroke-linecap="butt"/>' +
          '<text x="18" y="11.6" font-family="Arial, sans-serif" font-size="7" font-weight="700" fill="#CC8400" text-anchor="middle" dominant-baseline="middle">OHT</text>' +
        '</g>' +
    ...
    const drawing = window.SitePlanPolylineTool.create({
      ...
      buttonId:    BUTTON_ID,
      category:    'utilities',
      label:       'Overhead communication',
      idPrefix:    'communicationline',
      order:       35,
      proposedMode: true,
      iconApply:   window.SitePlanDrawingMode.iconSwapApply,
      iconClass:   'dm-line36',
      buttonTitle: 'Draw an overhead communication line',
      iconHtml:    communicationLineIcon,
      ...
      applyExtraMetadata: graphic => {
        if (US.ensureSitePlanId) {
          const id = US.ensureSitePlanId(graphic, 'communicationline');
    ...
    window.startCommunicationLineTool    = drawing.start;
    window.SitePlanCommunicationLineTool = Object.assign({}, window.SitePlanCommunicationLineTool || {}, api);

    if (typeof US.registerTool === 'function') {
      US.registerTool(window.SitePlanCommunicationLineTool);
    } else {
      console.warn('[tools-utilities/communication-line] Utilities registry unavailable.');
    }

  }).catch(err => {
    console.error('[tools-utilities/communication-line] Failed to initialize after runtime ready:', err);
  });
}());
